...
def convert_one_file(
    ...
    signal, sr = datautils.load_audio(audio_path, mono=mono, sr=sr)
    ...
    padded_signal[:use_shape[0],
                  :use_shape[1]] = signal[:use_shape[0], :use_shape[1]]

    layers = datautils.make_layered_melgram(padded_signal,
                                            sr,
                                            mels=mels,
                                            phase=phase)
    ...
    datautils.save_melgram(outfile,
                           layers,
...
def preprocess_dataset(
    ...
    if (already_split is True):
        ...
        class_names = datautils.get_class_names(path=inpath+"Train/")   # get the names of the subdirectories
        sampleset_subdirs = ["Train/","Test/"]
    elif nosplit:
        print(" All files output to same directory", flush=True)
        class_names = datautils.get_class_names(path=inpath)   # get the names of the subdirectories
        sampleset_subdirs = ["./"]
    else:
    ...
    max_shape = find_max_shape(inpath, mono=mono,
                               sr=resample,
                               dur=dur,
                               clean=clean)
    ...
    for subdir in sampleset_subdirs:  # non-class subdirs of Samples (in case already split into Test/ Train; see above)
        ...
        for class_index, classname in enumerate(class_names):   # go through the classes
            ...
            class_files = list(datautils.listdir_nohidden(dirname))   # all filenames for this class, skip hidden files
            class_files.sort()
